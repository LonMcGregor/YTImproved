...
(function () {
  ...
  var BUTTON_TEXT={'ar':'?????','cs':'Stáhnout','de':'Herunterladen','en':'Download','es':'Descargar','fr':'Télécharger','hi':'???????','hu':'Letöltés','id':'Unduh','it':'Scarica','ja':'??????','ko':'????','pl':'Pobierz','pt':'Baixar','ro':'Descarca?i','ru':'???????','tr':'Indir','zh':'??'};
  var BUTTON_TOOLTIP={'ar':'????? ??? ???????','cs':'Stáhnout toto video','de':'Dieses Video herunterladen','en':'Download this video','es':'Descargar este vídeo','fr':'Télécharger cette vidéo','hi':'?????? ??????? ????','hu':'Videó letöltése','id':'Unduh video ini','it':'Scarica questo video','ja':'??????????????','ko':'? ???? ????','pl':'Pobierz plik wideo','pt':'Baixar este vídeo','ro':'Descarca?i acest videoclip','ru':'??????? ??? ?????','tr': 'Bu videoyu indir','zh':'?????'};
  var DECODE_RULE=[];
